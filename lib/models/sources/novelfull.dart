...
class NovelFull {
  ...
  static Future<void> downloadNovel(Novel novel, List<Chapter> preDownloadedChapters) async {
    bool cancel = false;
    ...
    try {
      ...
      for (var i = 1; i <= totalPages; i++) {
        ...
        while (skip < pageChapters.length) {
          ...
          writeChaptersToDisk(novel.url, chapters);
          skip += batch;

          // SEND DOWNLOAD PERCENTAGE TO UI
          final percentage = double.parse((((chapters.length) / (novel.totalChapters ?? 0)) * 100).toStringAsFixed(2));
          updateNovelDownloadPercentage(novel, percentage);
        }
      ...
      final epub = await generateEPUB(novel, chapters);
  ...
  static Future<void> downloadNovelWithIsolate(Map<String, Object> params) async {
...
updateNovelDownloadPercentage(Novel novel, double percentage) {
